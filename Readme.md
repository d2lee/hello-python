...
이 명령어를 실행한 결과중에 `hello-python`의 서비스의 external IP 란에 있는 도메인 주소를 복사한 뒤에 웹 브라우저에서 오픈합니다. 처음 앱을 배포한 경우라면 load balancer인 ELB를 설치하는데 수분이 걸릴 수 있습니다. 만약 웹 브라우저에서 오픈했을 때 에러가 발생하면 1-2분 뒤에 다시 오픈합니다. 
